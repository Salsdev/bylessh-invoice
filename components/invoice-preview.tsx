...
const InvoicePaper = forwardRef<HTMLDivElement, { isMobile?: boolean }>(
  ({ isMobile }, ref) => {
    ...
    return (
      ...
      >
        {/* ── Letterhead ── */}
        <div
          className={`bg-white flex justify-between items-center border-b border-line-light ${
            isMobile
        ...
        >
          ...
          <img
            src={LOGO_SRC}
            alt="bylessh logo"
            className={`object-contain block print:h-32 print:w-auto ${
              isMobile ? "h-24 w-auto" : "h-32 w-auto"
            }`}
          />
          <div
            className={`text-brand leading-[1.9] shrink-0 ${
              isMobile
        ...
        {/* ── Brand Stripe ── */}
        <div
          className={`bg-brand-dark flex justify-between items-center ${
            isMobile
        ...
        >
          <div
            className={`font-extrabold text-white tracking-[0.28em] uppercase ${
        ...
        <div className={isMobile ? "px-4 py-4" : "px-7 py-[22px]"}>
          ...
          <div>
            ...
            {customer.name ? (
              <>
                ...
                <div
                  className={`text-ink-muted leading-[1.85] mt-0.5 ${
                    isMobile ? "text-[10px]" : "text-[10.5px]"
                  }`}
                >
          ...
          <div className="flex mt-[18px] justify-end">
            <div className={isMobile ? "w-full" : "w-[200px]"}>
              <div
                className={`flex justify-between text-ink-muted py-1 ${
                  isMobile ? "text-xs" : "text-[11.5px]"
                }`}
              >
                <span>Subtotal</span>
                <span>{fmt(sub)}</span>
              ...
              {discPct > 0 && (
                <div
                  className={`flex justify-between text-[#c05828] py-1 ${
                    isMobile ? "text-xs" : "text-[11.5px]"
                  }`}
                >
              ...
              <div
                className={`flex justify-between items-center font-extrabold text-brand border-t border-brand/20 pt-4 mt-2 ${
          ...
          {(notes || biz.acct) && (
            <div className="mt-[18px] pt-4 border-t border-line-light">
              ...
              {notes && (
                <div
                  className={`text-ink-faint leading-[1.85] whitespace-pre-line ${
                    isMobile ? "text-[10px]" : "text-[9.5px]"
                  }`}
                >
                  {notes}
                </div>
              ...
              {biz.acct && (
                <div
                  className={`bg-surface-warm border-l-[3px] border-brand/30 py-2.5 px-3 rounded-r text-ink font-semibold mt-2.5 ${
    ...
    <main
      className={`flex-1 overflow-y-auto flex flex-col items-center gap-3
        print:p-0 print:bg-white print:h-auto print:overflow-visible ${
          isMobile ? "p-3 pb-4" : "p-5"
        }`}
    >
      {/* ── Preview header: Label on left, Export Dropdown on right (desktop only) ── */}
      <div className="w-full max-w-[540px] flex items-center justify-between print:hidden">
        <div
          className={`text-[9.5px] font-bold tracking-[0.14em] uppercase text-ink-faint ${
            isMobile ? "text-[10px]" : ""
          }`}
        >
          {isMobile
      ...
      {/* Visible paper preview */}
      <InvoicePaper isMobile={isMobile} ref={isMobile ? null : paperRef} />

      {/* Hidden desktop-style paper preview (only on mobile) so we export standard size, not broken mobile view */}
      {isMobile && (
        <div className="absolute -left-[9999px] -top-[9999px] w-[540px] pointer-events-none print:hidden bg-white">
          <InvoicePaper isMobile={false} ref={exportPaperRef} />
        </div>
      )}
